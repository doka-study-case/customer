...
@Service
@Slf4j
public class SalaryPaymentTransactionService implements GenericTransactionService {

    private final AccountService accountService;
    private final FeeCalculationService feeCalculationService;
    private final EventProducer eventProducer;

    public SalaryPaymentTransactionService(AccountService accountService, FeeCalculationService feeCalculationService, EventProducer eventProducer) {
    ...
    public BigDecimal calculateFee(CustomerType customerType, BigDecimal amount) {
        float ratio = feeCalculationService.getRatio(TransactionType.SALARY_PAYMENT, customerType);
    ...
    public void makeTransfer(CustomerType customerType, AccountEntity sourceAccountEntity,
        ...
        // outgoing salary payment record for source customer
        QueueTransaction sourceQueueTransaction = QueueTransaction.builder()
                ...
                .transferType(TransferType.OUTGOING_SALARY_PAYMENT)
                .build();
        ...
        // incoming salary payment transaction for target customer
        QueueTransaction targetQueueTransaction = QueueTransaction.builder()
                .customerId(targetAccountEntity.getCustomerId())
                .amount(transactionDto.getAmount())
                .sourceAccountId(sourceAccountEntity.getId())
                .targetAccountId(targetAccountEntity.getId())
                .transferType(TransferType.INCOMING_SALARY_PAYMENT)
                .build();
        eventProducer.sendTransaction(targetQueueTransaction);

        // incoming salary notification for target customer
        QueueNotification queueNotification = QueueNotification.builder()
                .customerId(targetAccountEntity.getCustomerId())
                .message("You have received a SALARY PAYMENT with amount of " + transactionDto.getAmount())
                .build();
